---
title: Fate Really Accelerated Edition
author: Ben Pringle
---
...
-   Fate Dice^TM^, at least four, preferably four per person. These are a special
...
the [gamemaster](#being-gamemaster) or GM. The GM's job is to present challenges to
...
the [**next chapter**](#who-do-you-want-to-be).
...
### Aspects in a Nutshell {#aspects-in-a-nutshell}
...
about the setting**](#establishing-facts), such as the presence of magic or the
...
in [*Aspects and Fate Points*](#aspects-and-fate-points)---but for now, this
should help you get the idea.

### High Concept {#high-concept-cc}
...
-   [Write two aspects](#aspects-in-a-nutshell): a high concept and a trouble.
-   Write another aspect.
-   Give your character a name and describe their appearance.
-   [Choose approaches](#approaches-character-creation).
-   Set your refresh to 3.
-   You may write up to two more aspects and [choose a
    stunt](#stunts-refresh-character-creation) if you wish, or you may do that during
play.

* * * * *

### Approaches {#approaches-character-creation}
...
talk about what each approach means and how you use them in [*How to Do Stuff:
Outcomes, Approaches, and Actions*](#how-to-do-stuff).
...
### Stunts and Refresh {#stunts-refresh-character-creation}
...
talk more about stunts in [*Stunts*](#stunts-chapter). Choose one stunt to
start, or you can wait and add a stunt during the game. Later, when your
...
How To Do Stuff: Outcomes, Actions, and Approaches {#how-to-do-stuff}
==================================================
...
appropriate [aspect](#aspects-and-fate-points). Often, the ability to use an
aspect to make something true in the story is bonus enough!
...
number, which is either a [fixed difficulty](#setting-difficulty) or the result
of the GM's roll for an NPC. Based on that comparison, your outcome is:
...
-   Create a new [situation aspect](#what-kinds-of-aspects).
...
-   **If you fail:** Either you don't create or discover the aspect at all, or
    you create or discover it but anopponent gets to invoke the aspect for
    free. The second option works best if the aspect you create or discover is
    something that other people could take advantage of (like Rough Terrain).
    You may have to reword the aspect to show that it benefits the other
    character instead of you---work it out in whatever way makes the most sense
    with the player who gets the free invocation. You can still invoke the
    aspect if you'd like, but it'll cost you a fate point.
-   **If you tie:** If you're creating a new aspect, you get
    a [**boost**](#what-kinds-of-aspects). Name it and invoke it once for
    free---after that, the boost goes away. If you're trying to discover an
    existing aspect, treat this as a success (see below).
...
-   **If you succeed with style**: You create or discover the aspect, and you
    or an ally may invoke it twicefor free. Usually you can't invoke the same
...
-   **If you fail**: You don't get any additional benefit from the aspect. You
    can still invoke it in the future if you'd like, at the cost of a fate
    point.
...
-   **Succeed with Style:** Attack hits and causes damage. May reduce damage by
    one to generate a boost.
...
You use the overcome action when you have to
...
situation aspect](#what-kinds-of-aspects) is usually an overcome
...
-   **If you fail**: You have a tough choice to make. You can simply fail---the
    door is still locked, the thug still stands between you and the exit, the
    enemy spaceship is still On Your Tail. Or you can succeed, but at a serious
...
-   **If you succeed**: You accomplish what you were trying to do. The lock
    springs open, you duck around the thug blocking the door, you manage to
    lose the alien spaceship on your tail.
...
-   If you succeed: Your attack hits and you do damage. See [*Ouch! Damage,
    Stress, and Consequences*](#ouch-damage).
-   If you succeed with style: You hit and [do damage](#ouch-damage), plus you
...
As we mentioned in [Who Do You Want to Be?](#who-do-you-want-to-be), there are
...
aspects](#aspects-and-fate-points)---we'll talk about this a lot in Aspects and
Fate Points.
...
action [against a difficulty level](#setting-difficulty) that the GM
...
GM [creates a new situation aspect](#what-kinds-of-aspects)reflecting this
change and puts it into play.
...
5.  On other people's turns, defend against or respond to their actions as
    necessary.
...
[situation aspects](#what-kinds-of-aspects), too.
...
one of the [four actions](#actions-chapter). Resolve the action to determine the
outcome. The conflict is over when only one side has characters still in the
fight.

Ouch! Damage, Stress, and Consequences {#ouch-damage}
======================================
...
Getting Better---Recovering from Stress and Consequences {#getting-better}
--------------------------------------------------------
...
-   Severe consequence: Clear it at the end of
    the [*scenario*](#build-scenarios), provided it makes sense within the
    story.
...
You spend fate points---which you keep track of with pennies or glass beads or
...
have [more than your refresh](#stunts-refresh-character-creation), you start
the next session with the fate points you ended this session with.
...
What Kinds of Aspects Are There? {#what-kinds-of-aspects}
--------------------------------
...
your [high concept](#high-concept-cc) and [trouble](#trouble-cc). They describe
...
happen when you get hit by attacks. [They go away slowly](#getting-better), as
...
Free invocations: Sometimes you can invoke an aspect for free, without paying a
fate point. If you create or discover an aspect through the create an
advantage action, the first invocation on it (by you or an ally) is free (if
you succeeded with style, you get two freebies). If you cause a consequence
through an attack, you or an ally can invoke it once for free.  A **boost** is
...
### Establishing Facts {#establishing-facts}
...
certain situations, but sometimes it gives you some other ability
...
A significant milestone usually occurs at the end of
a [scenario](#build-scenarios) or the conclusion of a big plot event (or,
when in doubt, at the end of every two or three sessions). Unlike minor
milestones, which are primarily about change, significant milestones are about
learning new things---dealing with problems and challenges has made your
character generally more capable at what they do.
...
Build Scenarios and Run Game Sessions {#build-scenarios}
-------------------------------------
...
-   Set difficulties: You decide how [difficult
    tasks](#setting-difficulty) should be.
...
Setting Difficulty Levels {#setting-difficulty}
-------------------------
...
Dr. Flushing is a fellow at the Institute for Gravitical and Electro-Mechanical
